...
/**  jobNodePath.getFullPath(node) = /$jobName/$node
 * Job node storage.
 */
public final class JobNodeStorage {
    ...
    public void executeInLeader(final String latchNode, final LeaderExecutionCallback callback) {
        try (LeaderLatch latch = new LeaderLatch(getClient(), jobNodePath.getFullPath(latchNode))) { //  $jobName/leader/election/latch
            latch.start();
            latch.await();
            callback.execute(); // org.apache.shardingsphere.elasticjob.lite.internal.election.LeaderElectionExecutionCallback
        //CHECKSTYLE:OFF
        } catch (final Exception ex) {
        //CHECKSTYLE:ON
            handleException(ex);
